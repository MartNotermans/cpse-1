...
void make_file(){
   std::ofstream outfile ("song.hpp");
   outfile << "#ifndef _FUR_ELISE_HPP\n#define _FUR_ELISE_HPP\n\n#include \"melody.hpp\"" <<std::endl;
   outfile << "class fur_elise : public melody {\npublic:\n   void play( note_player & p ){" <<std::endl;
   outfile.close();
}

void end_file(){
   std::fstream fs;
    fs.open ("song.hpp", std::fstream::app);
    fs << "    }\n};\n\n#endif";
    fs.close();
}
